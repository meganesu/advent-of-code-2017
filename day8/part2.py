...
def update_value_of_register(parsed_instruction, registers, highest_value_ever):
    register = parsed_instruction.register_name
    new_amount = get_value_of_register(parsed_instruction.register_name, registers) + parsed_instruction.amount
    registers[register] = new_amount
...
def main():
    ...
    registers = {}
    highest_value_ever = -999999999999999
    operators = {
        '==': operator.eq,
        '!=': operator.ne,
        '<': operator.lt,
    ...
    for line in input_from_file:
        ...
        if should_execute_instruction(parsed_instruction.condition, registers):
            new_amount = update_value_of_register(parsed_instruction, registers, highest_value_ever)
            if new_amount > highest_value_ever:
                highest_value_ever = new_amount
        # print('*****')
